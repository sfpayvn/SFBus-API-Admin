...
@Controller('admin/users/seller')
export class AdminUserSellerController {
  constructor(private adminUserSellerService: AdminUserSellerService) {}
  ...
  @Post('register')
  @Feature(MODULE_KEYS.USER_SELLER, FUNCTION_KEYS.USER_SELLER.CREATE)
  async register(
    ...
    try {
      ...
      return await this.adminUserSellerService.create(adminCreateUserDto, tenantId);
    } catch (error) {
  ...
  async updateProfile(
    ...
    try {
      const { tenantId } = user;
      const updatedUser = await this.adminUserSellerService.update(adminUpdateUserProfileDto, tenantId);
      return updatedUser;
    } catch (error) {
      throw new BadRequestException(error.message);
    }
  ...
  async setPasswordAsTemp(
    ...
    return this.adminUserSellerService.setPasswordAsTemp(userId, newPassword, tenantId);
  }

  @UseGuards(JwtAuthGuard, RolesGuard, QuotaGuard)
  @Roles(ROLE_CONSTANTS.ADMIN, ROLE_CONSTANTS.TENANT, ROLE_CONSTANTS.TENANT_OPERATOR)
  @UseInterceptors(QuotaHeadersInterceptor)
  @Feature(MODULE_KEYS.USER_SELLER, FUNCTION_KEYS.USER_SELLER.CREATE, ACTIONQUOTA_KEYS.RELEASED)
  @Delete(':id')
  delete(@Param('id', ParseObjectIdPipe) id: Types.ObjectId, @CurrentUser(ParseObjectIdPipe) user: UserTokenDto) {
    return this.adminUserSellerService.delete(id);
  }
  ...
  search(@Body(ParseObjectIdPipe) query: AdminSearchUsersQuery, @CurrentUser(ParseObjectIdPipe) user: UserTokenDto) {
    ...
    const { tenantId } = user;
    return this.adminUserSellerService.search(+pageIdx, +pageSize, keyword, sortBy, filters, tenantId);
  }
}
