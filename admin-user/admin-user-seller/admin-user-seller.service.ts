...
import { UserSellerService } from '@/module/core/user/user-seller/user-seller.service';

@Injectable()
export class AdminUserSellerService {
  constructor(
    @InjectModel(UserDocument.name) private userModel: Model<UserDocument>,
    @Inject(forwardRef(() => UserSellerService)) private readonly userSellerService: UserSellerService,
  ) {}

  async create(adminCreateUserDto: AdminCreateUserDto, tenantId?: Types.ObjectId): Promise<AdminUserDto> {
    return this.userSellerService.create(adminCreateUserDto, tenantId);
  }

  // Cập nhật thông tin người dùng
  async update(adminUpdateUserProfileDto: AdminUpdateUserProfileDto, tenantId: Types.ObjectId): Promise<boolean> {
    const result = await this.userSellerService.update(adminUpdateUserProfileDto, tenantId);
    return result != null;
  }
  ...
  async updateUserField(
    ...
    return this.userSellerService.updateUserField(userId, fieldName, value, tenantId);
  }
  ...
  async updatePassword(
    ...
    const result = await this.userSellerService.updatePassword(userId, adminUpdatePasswordUserDto, tenantId);
    return result != null;
  }

  async setPasswordAsTemp(userId: Types.ObjectId, tempPassword: string, tenantId: Types.ObjectId): Promise<boolean> {
    return this.userSellerService.setPasswordAsTemp(userId, tempPassword, tenantId);
  }

  delete(id: Types.ObjectId) {
    return this.userSellerService.delete(id);
  }
  ...
  async search(
    ...
    return this.userSellerService.search(pageIdx, pageSize, keyword, sortBy, filters, tenantId);
  }
}
